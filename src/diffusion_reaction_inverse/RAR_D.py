...
def main(NumDomain):
    ...
    for i in range(NumDomain-NumDomain//2):
        ...
        err_eq = np.power(Y, 2) / np.power(Y, 2).mean()
        err_eq_normalized = (err_eq / sum(err_eq))[:, 0]
        X_ids = np.random.choice(a=len(X), size=1, replace=False, p=err_eq_normalized)
        X_selected = X[X_ids]
    ...
    np.savetxt(f'error_u_RAR-D.txt', error_u)
    np.savetxt(f'error_k_RAR-D.txt', error_k)
